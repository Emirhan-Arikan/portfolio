...
class PasswordResetRequestView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger.error(f"PasswordResetRequest Exception: {e}")
            return Response({'error': 'Şifre sıfırlama talebi sırasında bir hata oluştu.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class PasswordResetConfirmView(APIView):
    ...
    def post(self, request):
        try:
            ...
            logger.error(f"PasswordResetConfirm Exception: {e}")
            return Response({'error': 'Şifre güncellenirken bir hata oluştu.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                logger.error(f"GitHub External Request Exception: {e}")
                return Response({'error': f"GitHub API bağlantı hatası: {str(e)}"}, status=status.HTTP_502_BAD_GATEWAY)

        except Exception as e:
            logger.error(f"GitHub Login View Exception: {e}")
            return Response({'error': 'Giriş işlemi sırasında sunucu hatası oluştu.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
    ...
    def post(self, request):
        try:
            ...
            try:
                ...
                logger.error(f"Google External Request Exception: {e}")
                return Response({'error': f"Google API bağlantı hatası: {str(e)}"}, status=status.HTTP_502_BAD_GATEWAY)

        except Exception as e:
            logger.error(f"Google Login View Exception: {e}")
            return Response({'error': 'Giriş işlemi sırasında sunucu hatası oluştu.'}, status=status.HTTP_500_INTERNAL_SERVER_ERROR)
